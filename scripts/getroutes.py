    ...
    for root, _, files in os.walk(folder):
        for file_name in files:
            if file_name.endswith('.py') and 'test' not in file_name:
                ...
                scan_file(path, relpath, routes, filter)
    return routes

ROUTE_REGEX = re.compile(r'(?:\W|_)routes?(?:\s*=\s*"([^"]*)"(?:\s+if.*\Welse\s+"([^"]*)")?|s*[(\[]\s*"([^"]*)")')

def scan_file(path, relpath, routes, filter):
        ...
        for i in range(len(lines)):
            ...
            if m:
                for i in range(1, 4):
                    key = m.group(i)
                    if key:
                        if filter is None or not filter.match(key):
...
if __name__ == '__main__':
    target = sys.argv[1] if len(sys.argv) in [2, 3] else None
    if not target:
        print("getroutes <folder> [filter]\n    filter, if present, is a regex of routes to ignore")
    filter = r"(^|/)identifiers($|/)" if len(sys.argv) == 2 else sys.argv[2]
    filter = re.compile(filter) if filter else None
    routes = get_routes(target, filter)
    print_routes(routes)